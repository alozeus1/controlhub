...
def _safe_get_jwt_claims():
    """Return jwt claims only when Flask-JWT context is active."""
    try:
        verify_jwt_in_request()
        return get_jwt()
    except Exception:
...
def me():
    ...
    return jsonify({
        ...
        "email": user.email,
        "role": user.role,
        "auth_provider": getattr(request, "auth_provider", user.auth_provider),
        "roles": getattr(request, "roles", [user.role]),
